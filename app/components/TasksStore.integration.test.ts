...
describe("TasksStore Integration", () => {
  ...
  test("should handle assignment state synchronization correctly", async () => {
    // Mock tasks.list to return a task with assignment data
    mockApiClient.post.mockImplementation((endpoint) => {
      if (endpoint === "/tasks.list") {
        const mockTasks = [
          {
    ...
    // Verify assignment data is properly loaded
    expect(tasksStore.all.length).toBe(1);
    const task = tasksStore.all[0];

    expect(task.assigneeCount).toBe(1);
    expect(task.assignees).toHaveLength(1);
    expect(task.assignees[0].id).toBe("user-456");
    expect(task.assignees[0].name).toBe("Test User");
    expect(task.assignments).toHaveLength(1);
    expect(task.assignments[0].userId).toBe("user-456");

    // Test filtering based on assignment
    expect(tasksStore.assigned.length).toBe(1);
    expect(tasksStore.unassigned.length).toBe(0);
    expect(tasksStore.assigned[0].id).toBe("task-1");
  });

  test("should prevent duplicate assignments and show proper error", async () => {
    // Create a task that's already assigned to the current user
    const assignedTask = new Task(
      {
        ...
        assigneeCount: 1,
        assignees: [
          { id: "current-user", name: "Current User", email: "current@example.com" },
        ],
        assignments: [],
      },
      tasksStore as never
    ...
    tasksStore.add(assignedTask);

    // Mock API to return "already assigned" error
    mockApiClient.post.mockImplementation((endpoint) => {
      if (endpoint === "/tasks.assign") {
        return Promise.reject({
          response: {
            status: 400,
            data: { error: "User is already assigned to this task" },
          },
        });
      }
      return Promise.reject(new Error("Unknown endpoint"));
    ...
    // Attempt to assign should fail with specific error
    await expect(tasksStore.assign(assignedTask, "current-user")).rejects.toThrow(
