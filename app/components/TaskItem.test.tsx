...
jest.mock("~/components/Flex", () => 
    function Flex({ children, ...props }: unknown) {
        return <div {...(props as Record<string, unknown>)}>{children as React.ReactNode}</div>;
    }
);

jest.mock("~/components/Text", () => 
    function Text({ children, ...props }: unknown) {
        return <span {...(props as Record<string, unknown>)}>{children as React.ReactNode}</span>;
    }
);
...
describe("TaskItem", () => {
    ...
    it("calls onCancel when cancel button is clicked", () => {
        ...
        render(
            <Provider auth={authStore} {...mockStores}>
                <TaskItem {...defaultProps} isEditing={true} onCancel={onCancel} {...props} />
            </Provider>
        );
