...
describe("TaskAssignment model", () => {
    ...
    describe("soft delete (paranoid)", () => {
        it("should soft delete by default", async () => {
            ...
            const deletedAssignment = await TaskAssignment.findByPk(assignmentId, { paranoid: false });
    ...
    describe("scopes", () => {
        it("should include user and assignedBy in default scope", async () => {
            const assignment = await buildTaskAssignment();
            const assignmentWithIncludes = await TaskAssignment.findByPk(assignment.id);
            ...
            expect(assignmentWithIncludes.assignedBy.id).toEqual(assignment.assignedById);
        });
    });
    ...
    describe("queries", () => {
        it("should find assignments by task", async () => {
            ...
            const assignment1 = await buildTaskAssignment({ taskId: task.id, userId: user1.id });
            const assignment2 = await buildTaskAssignment({ taskId: task.id, userId: user2.id });
        ...
        it("should find assignments by user", async () => {
            ...
            const assignment1 = await buildTaskAssignment({ taskId: task1.id, userId: user.id });
            const assignment2 = await buildTaskAssignment({ taskId: task2.id, userId: user.id });
